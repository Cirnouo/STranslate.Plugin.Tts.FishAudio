...
- **합성 제어**: 속도, 음량, 라우드니스 정규화, MP3 비트레이트, 표현력, 다양성, 지연 모드, 텍스트 정규화, 컨텍스트 연동을 지원합니다
...
> [!TIP]
> S2.1 Pro 모델은 `2026-07-31` UTC 종료 시점까지 기간 한정으로 무료입니다. 플러그인에서 `s2.1-pro-free` 모델을 선택해 사용합니다. 자세한 내용: [Fish Audio S2.1 Pro: Free Text-to-Speech API for Developers](https://fish.audio/blog/s2-1-pro-free-api/)
...
| 합성 모델 | 무료 기간에는 `s2.1-pro-free`, 이후에는 `s2.1-pro` | 2026-07-31 UTC 종료 시점까지 `s2.1-pro-free`, `s2.1-pro`, `s2-pro`, `s1` 선택 가능. 2026-08-01 UTC부터 무료 모델은 숨겨집니다. |
...
| 컨텍스트 연동 | 켜짐 | 같은 합성 오디오 안의 앞선 조각을 사용해 보이스 일관성을 유지하며, 이전에 생성한 다른 오디오는 참조하지 않습니다. |
