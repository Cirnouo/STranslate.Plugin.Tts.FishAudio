...
- **커버 캐시** — 보이스 커버 이미지를 STranslate 플러그인 캐시에 저장하고, 사용량은 `cover_images`의 실제 `.jpg` 파일을 스캔해 계산하며 설정에서 명시적인 정리 버튼으로 정리 가능
...
| 커버 캐시 | 자동 | `cover_image`를 `<보이스 ID>.jpg`로 플러그인 캐시에 저장합니다. 사용량은 `cover_images/*.jpg`를 스캔해 계산하며, "기타"의 "캐시 지우기" 버튼으로 정리 가능 |
