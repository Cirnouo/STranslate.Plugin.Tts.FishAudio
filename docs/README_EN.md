...
- **Cover cache** — Voice cover images are cached in STranslate's plugin cache directory, with usage and an explicit cleanup button in settings
...
| Cover Cache | Auto | Stores `cover_image` as `<voice ID>.jpg` in the plugin cache directory; clear it with the "Clear cache" button under "Misc" |
