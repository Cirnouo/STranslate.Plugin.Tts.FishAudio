...
**Date:** 2026-07-23
**Context:** Fish Audio extended the original `s2.1-pro-free` promotion, which had ended at `2026-07-24T00:00:00Z`, through the full UTC day of July 31. The runtime policy, model descriptions, README files, and API notes must all switch at the same instant so the plugin does not hide a still-available model or advertise it after expiry.
**Decision:** Keep the cutoff centralized in `FishAudioRuntime` and move it to `2026-08-01T00:00:00Z`. Every instant before the cutoff, including the final tick of `2026-07-31` UTC, retains `s2.1-pro-free` in the available model list and uses it as the default; at and after the cutoff the plugin hides it and defaults to `s2.1-pro`. User-facing copy describes July 31 as fully available in UTC, while the changelog records the extension from July 24 to July 31.
**Affects:** `FishAudioRuntime`, language resources, README files, `docs/api-tts.md`, changelog, regression tests.
...
**Affects:** `Configuration/Settings`, `CachedVoiceInfo`, `SettingsSchema`, `SettingsJsonConverter`, `SettingsMigrator`, `SettingsNormalizer`, `SettingsValidation`, `SettingsStore`, `Main`, `SettingsViewModel`, `FishAudioRuntime`, regression tests.
...
**Affects:** `PreviewAudioUrlValidator`, `SettingsViewModel`, `VoiceSearchItem`, `FishAudioRuntime`, language resources, API documentation, regression tests.
