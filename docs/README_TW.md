...
> [!TIP]
> S2.1 Pro 模型限時免費至 `2026-08-31`，在外掛內選用模型 `s2.1-pro-free` 進行呼叫。詳情入口：[Fish Audio S2.1 Pro：面向開發者的免費文字轉語音 API](https://fish.audio/blog/s2-1-pro-free-api/)
...
未傳入 `-Release` 時預設執行 Debug 建置。`-CleanOnly` 只執行清理，不進行建置。
