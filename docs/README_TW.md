...
- **封面快取** — 聲音封面圖快取到 STranslate 的外掛快取目錄，占用大小按 `cover_images` 中實際 `.jpg` 檔案掃描計算；清理時顯示旋轉等待提示並防止重複點擊
- **手動輸入驗證** — 直接輸入聲音 ID 驗證並載入聲音資訊
- **情緒標記** — 透過文字內聯標記控制語音情緒（S2-Pro: `[laugh]`，S1: `(happy)`）
- **韻律控制** — 語速（0.5x ~ 2.0x）、音量（±10 dB，0.1 dB 精度）、響度歸一化
- **生成參數** — 表現力、多樣性、延遲模式（品質優先 / 平衡 / 低延遲）、文字規範化、上下文關聯
...
| MP3 比特率 | `192 kbps` | 64 / 128 / 192 kbps |
| 表現力 | `0.7` | 0 ~ 1，越高越多樣 |
| 多樣性 | `0.7` | 0 ~ 1 |
| 延遲模式 | 品質優先 | 品質優先 / 平衡 / 低延遲 |
| 文字規範化 | 關閉 | 數字→文字等自動轉換 |
| 上下文關聯 | 開啟 | 使用前序音訊作為上下文以保持聲音一致性 |
| 封面快取 | 自動 | 將 `cover_image` 以 `<聲音 ID>.jpg` 快取至外掛快取目錄；占用大小掃描 `cover_images/*.jpg` 計算，可在「其他」中透過「清理快取」按鈕清理，處理中會顯示等待提示 |
