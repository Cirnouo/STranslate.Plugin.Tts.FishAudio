...
- **封面快取** — 聲音封面圖快取到 STranslate 的外掛快取目錄，占用大小按 `cover_images` 中實際 `.jpg` 檔案掃描計算，並可透過顯式按鈕清理
...
| 封面快取 | 自動 | 將 `cover_image` 以 `<聲音 ID>.jpg` 快取至外掛快取目錄；占用大小掃描 `cover_images/*.jpg` 計算，可在「其他」中透過「清理快取」按鈕清理 |
