...
- **カバーキャッシュ** — ボイスのカバー画像を STranslate のプラグインキャッシュに保存し、設定から使用量確認と明示的な削除ボタンでの削除が可能
...
| カバーキャッシュ | 自動 | `cover_image` を `<ボイス ID>.jpg` としてプラグインキャッシュに保存し、「その他」の「キャッシュをクリア」ボタンで削除可能 |
