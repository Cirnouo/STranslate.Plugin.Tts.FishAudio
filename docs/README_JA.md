...
- **カバーキャッシュ** — ボイスのカバー画像を STranslate のプラグインキャッシュに保存し、使用量は `cover_images` 内の実際の `.jpg` ファイルをスキャンして計算、削除中は回転する待機表示を出して連続クリックを防止
...
| カバーキャッシュ | 自動 | `cover_image` を `<ボイス ID>.jpg` としてプラグインキャッシュに保存。使用量は `cover_images/*.jpg` をスキャンして計算し、「その他」の「キャッシュをクリア」ボタンで削除可能。処理中は待機表示を表示 |
