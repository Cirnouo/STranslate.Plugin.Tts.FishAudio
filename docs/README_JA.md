...
> [!TIP]
> S2.1 Pro モデルは `2026-08-31` まで期間限定で無料です。プラグイン内で `s2.1-pro-free` を選択して使用します。詳細: [Fish Audio S2.1 Pro: Free Text-to-Speech API for Developers](https://fish.audio/blog/s2-1-pro-free-api/)
...
`-Release` を指定しない場合は Debug ビルドが既定です。`-CleanOnly` はビルドせずに削除処理だけを実行します。
