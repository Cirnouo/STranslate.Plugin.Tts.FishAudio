...
> [!TIP]
> S2.1 Pro 模型限时免费至 `2026-08-31`，在插件内选用模型 `s2.1-pro-free` 进行调用。详情入口：[Fish Audio S2.1 Pro：面向开发者的免费文字转语音 API](https://fish.audio/blog/s2-1-pro-free-api/)
...
不传 `-Release` 时默认执行 Debug 构建。`-CleanOnly` 只执行清理，不进行构建。
