...
- **封面缓存** — 声音封面图缓存到 STranslate 插件缓存目录，占用大小按 `cover_images` 中实际 `.jpg` 文件扫描计算，并支持通过显式按钮一键清理
...
| 封面缓存 | 自动 | `cover_image` 以 `<声音 ID>.jpg` 缓存在插件缓存目录；占用大小扫描 `cover_images/*.jpg` 计算，可在“其他”中通过“清理缓存”按钮清理 |
