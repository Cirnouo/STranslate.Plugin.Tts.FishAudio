...
- Design decisions are recorded in `docs/DESIGN_DECISIONS.md` using ADR format. Add a new entry when introducing non-obvious behavior. **This file must be updated after each requirement batch**, alongside CHANGELOG.md, applicable locale files, and AGENTS.md; CLAUDE.md imports the same instructions through `@AGENTS.md`.
...
- `.claude/rules/`, `AGENTS.md`, and `CLAUDE.md` are tracked repository guidance. `AGENTS.md` is the canonical source; `CLAUDE.md` is a regular file containing the `@AGENTS.md` import so Claude Code loads the same instructions without duplicated content.
