...
public class Main : ITtsPlugin
{
    ...
    {
        IPluginContext operationContext;
        Settings operationSettings;
        SettingsViewModel? operationViewModel;
        IAudioPlayer operationAudioPlayer;
        lock (_stateGate)
        {
            var state = GetRequiredInitializationState();
            operationContext = state.Context;
            operationSettings = state.Settings;
            operationViewModel = state.ViewModel;
            operationAudioPlayer = state.AudioPlayer;
        }

        if (string.IsNullOrWhiteSpace(text))
        {
            operationContext.Snackbar.ShowWarning(
                operationContext.GetTranslation("STranslate_Plugin_Tts_FishAudio_Text_Empty"));
            return;
        }

        if (!FishAudioRuntime.TryPreflightApiKey(
                operationContext,
                operationSettings.ApiKey,
                "TTS",
                showError: true,
                out _))
        {
        ...
        operationViewModel?.BeginApiKeyOperation();
        try
        {
            var audioBytes = await FishAudioApi.PostTtsAsync(
                ...
                text,
                cancellationToken);
            await operationAudioPlayer.PlayAsync(audioBytes, cancellationToken);

            if (operationViewModel is not null)
                _ = operationViewModel.RefreshCreditSilentlyAsync();
        }
        catch (Exception ex) when (ex is not OperationCanceledException || !cancellationToken.IsCancellationRequested)
        {
            FishAudioRuntime.LogRequestFailure(operationContext, "TTS request", ex);
            var message = FishAudioRuntime.IsTimeout(ex)
                ? operationContext.GetTranslation(FishAudioRuntime.RequestTimeoutKey)
                : TryExtractApiError(ex) ?? ex.Message;
            operationContext.Snackbar.ShowError(message);
        }
        finally
        {
            operationViewModel?.EndApiKeyOperation();
        }
    }
